...
* Network container metrics:
    * A network container may either be the entire network processed in a run, or a section of the network (geographical region, subgeographical region, feeder,
      or LV feeder).
...
The following is an example of creating and populating an `IngestionJob` object.
